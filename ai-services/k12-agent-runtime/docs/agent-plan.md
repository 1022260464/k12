...
| --- | --- | --- | --- |
| `teaching-assistant` | AI 通识教学助手 | 根据学段、教材章节和问题进行分层讲解，主动引导并选择互动形式 | AI 通识知识库、模型调用、内容安全 |
...
3. **上线 AI 通识教学与练习辅导**：先使用小范围 AI 通识课程数据灰度，记录质量反馈、拒答率和工具失败率。
...
建议优先完成 `teaching-assistant` 的端到端最小版本：限定“冒泡排序”这一 AI 通识主题，对
小学高年级和高中采用不同解释与练习，输出“概念解释 + 互动示例 + 理解检查 + 下一步建议”，
并复用现有 `AgentRunResult`。随后再扩展 `ANIMATION`、`GAME` 和 `CODE_RESULT` 产物协议。
