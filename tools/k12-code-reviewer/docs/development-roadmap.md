...
负责教学解释、修改建议和追问。当前Reviewer只做本地静态审查，不接入Piston、Judge0或
云沙箱，也不运行用户代码。
...
- 接入Checkstyle教学规范。
- 优先实现不依赖字节码的源码规则。
- SpotBugs、`javac` 和其他需要构建用户项目的检查不进入当前阶段，后续单独评审。
...
- 增加规则超时、输入大小、输出大小和并发限制。
- 固定所有外部分析器版本，不读取用户项目中的插件和可执行配置。
- 建立误报样例库、规则基准测试和多语言回归测试。
- 完善规则启停、规则集版本和审计信息。
- 与代码沙箱保持独立部署和独立权限边界。
