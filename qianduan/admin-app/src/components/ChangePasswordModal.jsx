...
export function ChangePasswordModal({ onClose, notify }) {
      ...
      setFieldErrors((current) => ({
        ...
        ...(name === "newPassword" || name === "confirmPassword"
          ? {
            ...
            newPassword: name === "newPassword" ? nextErrors.newPassword : current.newPassword,
          }
          : {}),
        currentPassword: name === "currentPassword"
          ? nextErrors.currentPassword
          : (current.currentPassword?.includes("不正确") ? "" : current.currentPassword),
      }));
    }
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (mapped.form || mapped.currentPassword) {
        notify?.(mapped.form || mapped.currentPassword, "error");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <Modal
      title="修改登录密码"
      description={PASSWORD_CHANGE_RULES_TEXT}
      onClose={onClose}
      width={460}
      layer={130}
