...
import { Edit3, Plus, RefreshCw, Search, Trash2, UserRoundCheck, UsersRound } from "lucide-react";
  ...
  return (
    <section className="page-section">
      ...
      <div className="data-panel">
        ...
        <div className="table-wrap"><table><thead><tr><th>用户</th><th>角色</th><th>状态</th><th>更新时间</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{loading ? <tr><td colSpan="5" className="empty-cell">正在加载用户...</td></tr> : filtered.length === 0 ? <tr><td colSpan="5" className="empty-cell">没有匹配的用户</td></tr> : filtered.map((user) => <tr key={user.id}><td><div className="identity-cell"><span className="table-avatar">{(user.nickname || user.username).slice(0,1)}</span><div><strong>{user.nickname || user.username}</strong><small>{user.username} · {user.email || "未填写邮箱"}</small></div></div></td><td><div className="role-tags">{user.roleCode?.split(",").filter(Boolean).map((role) => <span key={role}>{roles.find((item) => item.code === role)?.name || role}</span>)}</div></td><td><span className={`status ${user.status?.toLowerCase()}`}>{user.status === "ENABLED" ? "正常" : user.status === "LOCKED" ? "锁定" : "停用"}</span></td><td>{user.updatedTime ? new Date(user.updatedTime).toLocaleString("zh-CN", { dateStyle: "medium", timeStyle: "short" }) : "-"}</td><td><div className="row-actions"><button className="icon-button" type="button" title="编辑用户" onClick={() => setEditing(user)}><Edit3 size={17} /></button><button className="icon-button danger" type="button" title="删除用户" onClick={() => setDeleting(user)}><Trash2 size={17} /></button></div></td></tr>)}</tbody></table></div>
      </div>
