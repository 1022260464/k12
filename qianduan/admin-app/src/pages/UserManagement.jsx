...
export function UserManagement({ roles, notify }) {
  ...
  async function resetPassword(event) {
    ...
    try { await usersApi.resetPassword(passwordTarget.id, newPassword); setPasswordTarget(null); setNewPassword(""); notify("密码已重置，用户原有令牌将失效"); }
    catch (error) { notify(error.message, "error"); }
  }
  ...
  return (
    <section className="page-section">
      ...
      {passwordTarget && <Modal title="重置用户密码" description={`为 ${passwordTarget.username} 设置新密码，成功后该用户现有 JWT 将失效。`} onClose={() => setPasswordTarget(null)} width={460}><form className="resource-form" onSubmit={resetPassword}><label>新密码<input type="password" minLength="8" maxLength="72" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} required /></label><div><button className="button ghost" type="button" onClick={() => setPasswordTarget(null)}>取消</button><button className="button primary" type="submit">确认重置</button></div></form></Modal>}
    </section>
  );
}
