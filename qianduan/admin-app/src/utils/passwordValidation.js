...
export function validateCreateUserForm({ username, password, nickname, email }) {
  const usernameError = validateUsername(username);
  if (usernameError) return usernameError;
  if (!String(nickname || "").trim()) return "请输入显示名称";
  if (/[<>"'`\\]/.test(nickname || "")) return "显示名称不能包含特殊符号 < > \" ' ` \\";
  const emailValue = String(email || "").trim();
  if (emailValue && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailValue)) return "邮箱格式不正确";
  return validateNewPassword(password, { label: "初始密码" });
}
...
export function mapUniqueIdentityError(message) {
  ...
  return { form: msg || "暂时无法完成操作，请稍后再试", fields };
}
